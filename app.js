...
function randomNext(){
  ...
  if(visibleTracks.length===1) return visibleTracks[0];
  const i=queueIndex(); let n=i;
  while(n===i) n=Math.floor(Math.random()*visibleTracks.length);
  return visibleTracks[n];
}
function nextTrack(){
  if(!visibleTracks.length) return;
  if(shuffleMode){ playTrack(randomNext()); return; }
  const i=queueIndex(); playTrack(visibleTracks[(i<0?0:i+1)%visibleTracks.length]);
}
$('playBtn').onclick=()=>{ if(!currentPath && visibleTracks.length) playTrack(shuffleMode?randomNext():visibleTracks[0]); else if(audio.paused) audio.play(); else audio.pause(); };
$('prevBtn').onclick=()=>{ if(!visibleTracks.length)return; if(shuffleMode){playTrack(randomNext());return;} const i=queueIndex(); playTrack(visibleTracks[i<=0?visibleTracks.length-1:i-1]); };
